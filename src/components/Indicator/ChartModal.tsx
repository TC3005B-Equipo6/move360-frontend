...
export const ChartModal = ({
  ...
  useEffect(() => {
    if (currentSource?.datasets?.length) {
      setDatasetId(
        currentSource.datasets[0].id
      );
    }
  }, [source]);

  const currentDataset =
    currentSource?.datasets.find(
      (d) => d.id === datasetId
    );
  ...
  const [compareEnabled, setCompareEnabled] =
    useState(false);
  const [compareTable, setCompareTable] =
    useState("");
  ...
  const handleSave = () => {
    ...
    const series = selectedColumns.map(
      (column, index) => ({
        key: column,
        label: column,
        color:
      ...
      <div className="flex-1 overflow-y-auto pr-2 flex flex-col gap-5">
        <div className="grid grid-cols-3 gap-5">
          <div className="flex flex-col gap-2">
            <label className="text-sm font-semibold text-[#1F4E79]">
              Inicio
            </label>
            <input
              type="month"
        ...
        <div className="rounded-3xl border border-[#DCE4EE] overflow-hidden bg-white">
          <div className="px-6 py-4 border-b border-[#DCE4EE] bg-[#F8FBFF]">
            <h3 className="font-bold text-[#1F4E79] text-lg">
            ...
            <table className="w-full text-sm">
              <thead className="bg-[#F8FBFF] sticky top-0">
                <tr>
                  {selectedColumns.map(
                    (column) => (
                      <th key={column} className="px-4 py-3 text-left text-[#1F4E79] border-b border-[#DCE4EE]"  > {column}
                      </th>
                    )
                  )}
                </tr>
